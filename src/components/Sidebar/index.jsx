      ...
      <div className="flex items-center justify-between gap-2 px-5 py-5">
        <NavLink to="/">
          <div className='flex items-center justify-center'>
            ...
            <h6 className='text-white text-lg text-center uppercase tracking-wider'>University BAC Office</h6>
          </div>
        </NavLink>
...

              {
                sideItems.map((item) => (
                  item.dropdown ? <>
                     <SidebarLinkGroup
                        activeCondition={
                          pathname === item.url || pathname.includes(`${item.includes}`)
                        }
                      >
                        {(handleClick, open) => {
                          return (
