...
export default function EditCategoryModal({ open, handleModal, loading, handleBntLoading, handlePageLoading, category}) {
  ...
  const saveDepartment = async (ev) => {
    ...
    try {
      const {data} = await axiosClient.post(`/category/update/${category.id}`, payload, config)
      // setDepartments(data)
      setNotification('Category Edited Successfully')
  ...
  const getDepartment = async () => {
    try {
      const {data} = await axiosClient.get(`/category/show/${category.id}`)
      setPayload({
        name: data.name,
        status: data.status,
      })
          ...
          <div className='mt-5'>
            <div className='mb-4'>
              ...
              <input
                ...
                value={payload.name}
                onChange={onChange}
                className={`${errors.name ? 'border-red-500' : 'border-stroke'} w-full rounded-lg border-[1.5px] bg-transparent py-3 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary`}
              />
              {
                errors.name ? <p className='text-red-500 italic'><i className="fa-solid fa-circle-exclamation"></i> {errors.name}</p> : null
              }
            </div>
