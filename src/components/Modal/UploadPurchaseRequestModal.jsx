...
export default function UploadPurchaseRequestModal({ open, handleModal, loading, handlePageLoading}) {
  ...
  const handlePdfChange = (ev) => {
    setPayload({...payload, purchase_request: ev.target.files[0]})
  ...
  const saveDepartment = async (ev) => {
    ...
    setBtnLoading(true)
    try {
      const {data} = await axiosClient.post('/po-request/store', payload, config)
    ...
    <>
      ...
      <dialog className={`modal ${open ? 'modal-open' : ''}`}>
        <div className="modal-box">
          <h3 className="font-bold text-sm uppercase">Upload Purchase Request</h3>
          ...
          <div className='mt-5'>
            <div className='mb-4'>
              ...
              <input
                ...
                value={payload.request_description}
                onChange={onChange}
            ...
            <div className='mb-4 w-full'>
              ...
              <UploadPdf handlePdfChange={ev => handlePdfChange(ev)} pdf={pdfurl} error={errors.purchase_request}/>
            </div>
          </div>
