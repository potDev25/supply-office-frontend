...
export default function AddSupplierModal({ open, handleModal, loading, handleBntLoading, handlePageLoading}) {
  ...
  const saveDepartment = async (ev) => {
    ...
    try {
      const {data} = await axiosClient.post('/suppliers/store', payload, config)
      // setDepartments(data)
      setSupplier(data)
    ...
    <>
      ...
      <dialog className={`modal ${open ? 'modal-open' : ''}`}>
        <div className="modal-box">
          <h3 className="font-bold text-sm uppercase">Add Supplier</h3>
