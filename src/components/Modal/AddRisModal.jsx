...
export default function AddRisModal({ open, handleModal, loading, handleBntLoading, handlePageLoading}) {
  ...
  const saveDepartment = async (ev) => {
    ...
    try {
      const {data} = await axiosClient.post('/ris/store', payload, config)
      setNotification('RIS Added Successfully')
    ...
    <>
      ...
      <dialog className={`modal ${open ? 'modal-open' : ''}`}>
        <div className="modal-box relative">
          <h3 className="font-bold text-sm uppercase text-center">Add Requisition and Issue Slip ?</h3>
            ...
            <button className="btn bg-primary text-white" onClick={saveDepartment} disabled={loading ? true : false}>
              {
                ...
                <i class="fa-solid fa-share-from-square"></i> ADD
                </>
              }
            </button>
          </div>
