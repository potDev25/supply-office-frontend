...
export default function AddParModal({ open, handleModal, loading, handleBntLoading, handlePageLoading}) {
  const [errors, setErrors] = useState([])
  const {setNotification, setNotificationError, setDepartments, users} = useStateContext()
  ...
  const saveDepartment = async (ev) => {
    ...
    try {
      const {data} = await axiosClient.post('/par/store', payload, config)
      setNotification('PAR Added Successfully')
    ...
    <>
      ...
      <dialog className={`modal ${open ? 'modal-open' : ''}`}>
        <div className="modal-box relative">
          <h3 className="font-bold text-sm uppercase">ADD PAR</h3>
          ...
          <div className='mt-5 relative'>
            ...
            <div className='mb-4 relative'>
              ...
              <Select
                options={users}
                styles={customStyles}
                onChange={ev => setPayload({...payload, client_id: ev.value})}
              />
              {
                errors.client_id ? <p className='text-red-500 italic'><i className="fa-solid fa-circle-exclamation"></i> {errors.client_id}</p> : null
              }
            </div>
          </div>
