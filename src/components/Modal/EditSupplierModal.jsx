...
export default function EditSupplierModal({ open, handleModal, loading, handleBntLoading, handlePageLoading, category}) {
  const [errors, setErrors] = useState([])
  const {setNotification, setNotificationError, setDepartments, setSupplier} = useStateContext()
  ...
  const saveDepartment = async (ev) => {
    ...
    try {
      const {data} = await axiosClient.post(`/suppliers/update/${category.id}`, payload, config)
      // setDepartments(data)
      setSupplier(data)
      setNotification('Supplier Edited Successfully')
  ...
  const getDepartment = async () => {
    try {
      const {data} = await axiosClient.get(`/suppliers/show/${category.id}`)
      setPayload({
    ...
    <>
      ...
      <dialog className={`modal ${open ? 'modal-open' : ''}`}>
        <div className="modal-box">
          <h3 className="font-bold text-sm uppercase">Edit Category</h3>
          ...
          <div className='mt-5'>
            <div className='mb-4'>
              ...
              <input
                ...
                onChange={onChange}
                className={`${errors.supplier_name ? 'border-red-500' : 'border-stroke'} w-full rounded-lg border-[1.5px] bg-transparent py-3 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary`}
              />
              {
                errors.supplier_name ? <p className='text-red-500 italic'><i className="fa-solid fa-circle-exclamation"></i> {errors.supplier_name}</p> : null
              }
            </div>
