...
export default function StoreSupplyModal({
  open,
  handleModal,
  loading,
  handleBntLoading,
  ...
  const saveDepartment = async (ev) => {
    ...
    try {
      const { data } = await axiosClient.post(
        `/stocks/store/${payload.supply_id}/${id}`,
        payload,
        config,
      );
      // setDepartments(data);
          ...
          <div className="mt-5">
            ...
            <div className="mb-4">
              ...
              <CurrencyInput
                id="input-example"
                name="input-name"
                placeholder="Please Enter Supply Price"
                decimalsLimit={2}
                defaultValue={payload.price}
                className={`${
                  errors.department_name ? 'border-red-500' : 'border-stroke'
                } w-full rounded-lg border-[1.5px] bg-transparent py-3 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary`}
                onValueChange={ev => setPayload({...payload, price: ev})
                }
              />
              {errors.price && (
                <p className="text-red-500 italic">
