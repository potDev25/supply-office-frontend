...
import AddReturnStatusModal from '../Modal/AddReturnStatusModal';

const ReturnStatusTable = () => {
  ...
  const {notification_error, setNotificationError, setNotification, setReturnStatus} = useStateContext()
  ...
  const fetchData = async () => {
    // setLoading(true)
    try {
      const response = await axiosClient.get(`/return-status?page=${page}&limit=${limit}`)
      setData(response.data.data)
      setReturnStatus(response.data.data)
  ...
  const handleAllDelete = async () => {
    if(ids.length > 0){
      setBtnLoading(true)
      try {
        await axiosClient.post('/return-status/batch-delete', ids);
        setLoading(true)
        setBtnLoading(false)
        setDeleteModal(false)
        setNotification('Return Status Deleted Successfully')
        setIds([])
      } catch (error) {
        console.log(error);
  ...
  const deleteSingleUser = async () => {
    setBtnLoading(true)
    try {
      await axiosClient.post(`/return-status/destroy/${user_id}`);
      setLoading(true)
      setBtnLoading(false)
  ...
  return (
    <div className="overflow-x-auto rounded-sm border border-stroke bg-white pt-2 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className='flex items-center justify-between mt-2 mb-2'>
        <div className='flex items-center gap-2'>
          <label className="input input-bordered flex items-center gap-2">
            <input type="text" className="grow input-xs" onChange={ev => setSearch(ev.target.value)} placeholder="Search Status" />
      ...
      <AddReturnStatusModal handlePageLoading={handlePageLoading} open={departmentModal} handleModal={handleDepartmentModal} loading={btnLoading} handleBntLoading={handleBtnLoading}/>

      {
        ...
        <table className="table table-zebra fade-in">
          ...
          <tbody>
            {
              data.filter((data) => {
                    return search.toLowerCase === '' ? data : data.return_status.toLowerCase().includes(search)
                }).map((data) => (
                <tr>
                  <th>
                    <label>
                  ...
                  <td>
                    <div className="flex items-center gap-3">
                      <div>
                        <div className="font-bold capitalize">{data.return_status}</div>
                      </div>
                    </div>
                  </td>
...
export default ReturnStatusTable;
