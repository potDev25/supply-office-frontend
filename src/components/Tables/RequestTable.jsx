  ...
  const fetchData = async () => {
    try {
      const { data } = await axiosClient.get(
        `/documents${status ? `?status=${status}` : ''}`,
      );
    ...
    <div className="lg:overflow-x-visible sm:overflow-x-auto md:overflow-x-auto rounded-sm border border-stroke bg-white pt-2 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1 mb-10">
      ...
      <label className="input input-bordered flex items-center gap-2 w-80 mb-4">
        <input
          type="text"
          className="grow"
          placeholder="Search Department Name | User"
      ...
      <ProceedModal
        ...
        handleModal={handleProceedModal}
        tableLoading={handleTableLoading}
        data={dataRequest}
      />
      <ReturnModal
        open={openReturnModal}
        handleModal={handleReturnModal}
        data={dataRequest}
        tableLoading={handleTableLoading}
      />
                    ...
                    <i class="fa-solid fa-file-lines"></i> {data.file_name}
                      ...
                      <td>
                        {data.document_status === 'for review' ? (
                          <div
                            className={`badge badge-default badge-outline capitalize text-xs`}
                          >
                            {data.document_status}
                          </div>
                        ) : null}
                        {data.document_status === 'consolidated' ? (
                          <div
                            className={`badge badge-success badge-outline capitalize text-xs`}
                          >
                            {data.document_status}
                          </div>
                        ) : null}
                        {data.document_status === 'supply office' ? (
                          <div
                            className={`badge badge-accent badge-outline capitalize text-xs`}
                          >
                            {data.document_status}
                          </div>
                        ) : null}
                        {data.document_status === 'return' ? (
                          <div
                            className={`badge badge-ghost badge-sm capitalize text-xs`}
                          >
                            {data.document_status}
                          </div>
                        ) : null}
                        {data.document_status === 'for consolidation' ? (
                          <div className="badge border border-red-500 text-red-500 badge-outline">
                            {data.document_status}
                          </div>
                        ) : null}
                      </td>
