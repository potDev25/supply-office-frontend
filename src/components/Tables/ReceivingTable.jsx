  ...
  const fetchData = async () => {
    ...
    try {
      const {data} = await axiosClient.get(
        `/receving?page=${page}&limit=${limit}`,
      );
      setData(data.data);
      setLinks(data.links);
  ...
  return (
    <div className="overflow-x-auto rounded-sm border border-stroke bg-white pt-2 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="flex items-center justify-between mt-2 mb-2">
        ...
        <div className="flex items-center gap-2">
          <Button colorScheme='blue' onClick={handleDepartmentModal} className='uppercase'><i class="fa-solid fa-circle-plus"> </i> {" "}Receiving</Button>
        </div>
      </div>
                ...
                <>
                  ...
                  .filter((data) => {
                    return search.toLowerCase === ''
                      ? data
                      : data.supplier.toLowerCase().includes(search) || data.doc_id.toLowerCase().includes(search)
                  })
                  .map((data) => (
                    <tr>
                      <td>
                      ...
                      <td className="capitalize">{data.supplier}</td>
                      <td className="capitalizen text-blue-500">{formatDate(data.date_arrived)}</td>
